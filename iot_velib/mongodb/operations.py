...
def update_station_information(stations, collection, batch_size=100):
    '''
    Bulk replace velib bike stations in MongoDB. New stations will be added automatically, changes in stations will be replaced.
    '''
...
def update_station_status(station_status, collection, metadata_collection, feed, batch_size=100):
    ...
    for station in station_status['data']['stations']:
        ...
        station_id = station.pop('stationCode')

        # Add the timestamp when the data arrived, we want a date format for better readability
        station['ts'] = datetime.fromtimestamp(station_status['lastUpdatedOther'])
        ...
        batched_operations.append(pymongo.UpdateOne(
            {
                ...
                # We expect to execute the loader every minute, so roughly one hour will be stored in one bucket
                'bucket_size': { '$lt': 60 }
            },
            {
                ...
                '$max': { 
                    'max_ts': station['ts'], 
                    'expire_on': station['ts'] + timedelta(hours=24) 
                },
    ...
    set_station_last_updated(collection=metadata_collection, feed=feed, last_updated=station_status['lastUpdatedOther'])
